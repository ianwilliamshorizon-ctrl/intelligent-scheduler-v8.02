...
const PrintableInvoice: React.FC<PrintableInvoiceProps> = ({ 
    ...
    const headerBlockTypes = new Set(['header_logo', 'document_meta']);
    const footerBlockTypes = new Set(['footer_legal']);

    const headerBlocks = blocks.filter(b => b.visible !== false && headerBlockTypes.has(b.type));
    const footerBlock = blocks.find(b => b.visible !== false && footerBlockTypes.has(b.type));
    const bodyBlocks = blocks.filter(b => b.visible !== false && !headerBlockTypes.has(b.type) && !footerBlockTypes.has(b.type));

    const renderBlockContent = (block: DocumentBlockConfig) => {
        ...
        const isLineItemsTable = block.type === 'tasks_packages' || block.type === 'parts_items';
        const breakInsideStyle = isLineItemsTable ? 'auto' : 'avoid';

        return (
            <div 
                key={block.id} 
                className={`transition-all ${block.settings?.pageBreakBefore ? 'page-break-before' : ''} ${!isLineItemsTable ? 'page-break-inside-avoid' : ''}`}
                style={{ 
                    breakInside: breakInsideStyle, 
                    pageBreakInside: breakInsideStyle,
            ...
            >
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            {vehicle?.registration && (
                                <span className="inline-block bg-yellow-400 text-black font-mono font-black text-xs px-2 py-0.5 rounded border border-yellow-500 shadow-2xs tracking-wider uppercase ml-1">
                                    {vehicle.registration}
                                </span>
                            )}
                        </div>
                    ...
                    <div className={cs.wrapperClass} style={cs.wrapperStyle}>
                        ...
                        <div className={cs.bodyClass}>
                            <div className="text-xs space-y-0.5" style={cs.textStyle}>
                                ...
                                <div className={customer?.companyName ? "text-xs font-semibold text-gray-700" : "font-bold text-sm"}>
                                    {customer ? `${customer.forename || ''} ${customer.surname || ''}`.trim() : 'Customer Cash Sale'}
                                </div>
                ...
                {block.type === 'tasks_packages' && (
                    <div className={cs.wrapperClass} style={cs.wrapperStyle}>
                        ...
                        <div className="p-0 overflow-visible print:overflow-visible">
                            <table className="w-full text-xs text-left" style={{ ...cs.textStyle, borderCollapse: 'collapse' }}>
                                <thead className="bg-slate-50/60 border-b text-[10px] uppercase print:bg-slate-100" style={{ ...cs.subtextStyle, display: 'table-header-group' }}>
                                    <tr>
                                    ...
                                    {groupedItems.labor.length > 0 || groupedItems.packages.length > 0 ? (
                                        <>
                                            {groupedItems.packages.map(pkg => (
                                                <tr key={pkg.header.id} className="bg-slate-50/40" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                ...
                {block.type === 'financial_totals' && (
                    <div className="flex justify-end pt-1 page-break-inside-avoid" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                ...
                {block.type === 'bank_details' && (
                    <div className={`${cs.wrapperClass} p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs page-break-inside-avoid`} style={{ ...cs.wrapperStyle, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                        <div>
                            <h4 className={cs.titleClass} style={cs.titleStyle}>
                ...
                {block.type === 'terms_signoff' && (
                    <div className={`${cs.wrapperClass} page-break-inside-avoid`} style={{ ...cs.wrapperStyle, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                        <div className={cs.headerClass} style={cs.headerStyle}>
                            <h4 className={cs.titleClass} style={cs.titleStyle}>
                ...
                {/* Block 12: Legal Footer (if rendered in body) */}
                {block.type === 'footer_legal' && (
                    <div className="pt-3 border-t border-slate-200 text-center text-[10px] text-slate-400 space-y-0.5 page-break-inside-avoid" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                        <div>
    ...
    return (
        <div className="rebuild-print-container" style={{ ...containerStyle, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
            ...
            {/* Main Multi-Page Printable Invoice Table Wrapper */}
            <table className="printable-page-wrapper w-full bg-white font-sans text-xs text-slate-800" style={{ width: '100%', borderCollapse: 'collapse', boxSizing: 'border-box' }}>
                {/* RUNNING HEADER (Repeats at top of every printed page) */}
                <thead className="print-header-group">
                    <tr>
                        <td style={{ padding: '12mm 14mm 4mm 14mm', verticalAlign: 'top' }}>
                            {headerBlocks.length > 0 ? (
                                <div className="space-y-3">
                                    {headerBlocks.map(block => renderBlockContent(block))}
                                </div>
                            ) : (
                ...
                {/* RUNNING FOOTER (Repeats at bottom of every printed page) */}
                <tfoot className="print-footer-group">
                    <tr>
                        <td style={{ padding: '4mm 14mm 10mm 14mm', verticalAlign: 'bottom' }}>
                            <div className="pt-2 border-t border-slate-300 text-xs text-slate-500">
                                <div className="flex justify-between items-end text-[10px]">
                                    <div className="space-y-0.5 max-w-[75%]">
                                        ...
                                        <div className="text-[9px] text-slate-500 italic">
                                            {footerBlock?.settings?.customFooterText || resolvedEntity?.invoiceFooterText || 'Registered in England & Wales. Thank you for your business.'}
                                        </div>
                                    </div>
                ...
                {/* INVOICE BODY CONTENT (Flows smoothly across pages) */}
                <tbody className="print-body-group">
                    <tr>
                        <td style={{ padding: '0 14mm', verticalAlign: 'top' }}>
                            <div className="space-y-4 pt-1 pb-4">
                                {bodyBlocks.map(block => renderBlockContent(block))}
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            {printOptions.showInspections && inspectionTemplate && job?.inspectionChecklist && job.inspectionChecklist.length > 0 && (
                <div className="bg-white font-sans text-sm text-gray-800 printable-additional-page" style={{ ...containerStyle, padding: '12mm 14mm', breakBefore: 'page', pageBreakBefore: 'always' }}>
