...
7. "notes": Any specific logistical considerations or turnaround times. This MUST be written in a friendly, customer-facing tone. CRITICAL: DO NOT leak, mention, or summarize any internal CRM notes, technician notes, profitability, costs, or any other non-customer-facing information here. The customer will read this directly. ABSOLUTELY EXCLUDE internal notes.
8. "explanation": A detailed, friendly, and customer-oriented markdown-formatted text explaining the estimate. Write this directly to the customer (e.g., "Hi there, here is the breakdown for..."). Explain the reasoning clearly, answer any questions, and break down labor, materials, and options without using internal jargon. CRITICAL: DO NOT leak or mention any internal notes, technician notes, CRM logs, action logs, costs, margins, or non-customer-facing context in this explanation. DO NOT include any prices, costs, or monetary values in this explanation, as the actual pricing will be provided in the final summary. IMPORTANT: Do NOT use raw double quotes (") inside this explanation string. Use single quotes (') instead of double quotes (") for highlighted terms, and ensure all newlines are written as escaped '\\n' characters rather than raw carriage returns/newlines.
...
User Request: ${JSON.stringify(userText)}`;
    ...
    const renderItemRow = (item: EstimateLineItem) => {
        ...
        if (isPackageHeader) {
            ...
            return (
                <div key={item.id} className="grid grid-cols-[minmax(0,1fr)_60px_80px_85px_95px_70px_36px] gap-2 items-center p-2 rounded-lg border bg-indigo-50 border-indigo-200 transition-all hover:shadow-md mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        <input 
                            ...
                            className="w-full bg-transparent border-none focus:ring-0 font-bold text-indigo-900 placeholder:text-indigo-300 text-sm truncate"
                    ...
                    <div>
                        <input 
                            ...
                            className="w-full p-1 border border-indigo-100 rounded text-right text-sm bg-white font-medium" 
                            placeholder="Sell" 
                        />
                    </div>
        ...
        return (
            <div key={item.id} className={`grid grid-cols-[minmax(0,1fr)_60px_80px_85px_95px_70px_36px] gap-2 items-start p-2 rounded-lg border text-sm transition-all shadow-sm ${isPackageComponent ? 'bg-gray-100' : 'bg-white border-gray-200'}`}>
                ...
                <div>
                    <input 
                        ...
                        value={item.quantity} 
                        onChange={e => handleLineItemChange(item.id, 'quantity', e.target.value)} 
                        className="w-full p-1 border rounded text-right text-sm bg-white self-center" 
                    />
                </div>
                <div>
                    <input 
                ...
                <div>
                    <input 
                        ...
                        value={item.unitPrice} 
                        onChange={e => handleLineItemChange(item.id, 'unitPrice', e.target.value)} 
                        className="w-full p-1 border rounded text-right text-sm bg-white font-medium self-center" 
                        placeholder="Sell" 
                    />
                </div>
                <div className="text-right text-sm font-bold text-slate-800 self-center truncate pr-1">
                    {isPackageComponent && (!item.unitPrice || Number(item.unitPrice) === 0) ? (
