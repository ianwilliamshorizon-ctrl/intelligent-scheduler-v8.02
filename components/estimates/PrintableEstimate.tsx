    ...
    const renderVehicleBlock = (alignment: 'left' | 'right' | 'center') => (
        <div style={{ 
            ...
            <p style={{ fontSize: '10px', fontWeight: 'bold', color: '#64748b', marginTop: '2px' }}>
                {[vehicle?.make && String(vehicle.make).toLowerCase() !== 'unknown' ? String(vehicle.make) : '', vehicle?.model && String(vehicle.model).toLowerCase() !== 'unknown' ? String(vehicle.model) : ''].filter(Boolean).join(' ')}
            </p>
        </div>
    );
