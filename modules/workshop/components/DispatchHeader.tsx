...
import { ChevronLeft, ChevronRight, Clock, PlusCircle, Printer } from 'lucide-react';
...
export const DispatchHeader: React.FC<DispatchHeaderProps> = ({
    ...
    const handlePrint = () => {
        ...
        // Inject landscape page style temporarily
        ...
        }, 500); // Remove class and style after a small delay to ensure print dialog renders correctly
    };

    return (
        <header className="calendar-header bg-white border-b p-3 sm:p-4 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 shadow-sm z-30">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">
                 <div className="flex bg-gray-200 rounded-lg p-1 w-full sm:w-auto overflow-x-auto no-scrollbar no-print">
                ...
                <div className="flex items-center gap-1 sm:gap-2 w-full sm:w-auto justify-between sm:justify-start">
                    {viewMode === 'timeline' && (
                     ...
                     {viewMode === 'week' && (
                         <>
                            <button onClick={() => setWeekStart(addDays(weekStart, -7))} className="no-print p-2 rounded-full hover:bg-gray-100 flex-shrink-0"><ChevronLeft size={18}/></button>
                            ...
                            <button onClick={() => setWeekStart(addDays(weekStart, 7))} className="no-print p-2 rounded-full hover:bg-gray-100 flex-shrink-0"><ChevronRight size={18}/></button>
                         </>
                    )}
                     {viewMode === 'calendar' && (
                         <>
                            <button onClick={() => handleMonthChange(-1)} className="no-print p-2 rounded-full hover:bg-gray-100 flex-shrink-0"><ChevronLeft size={18}/></button>
                            ...
                            <button onClick={() => handleMonthChange(1)} className="no-print p-2 rounded-full hover:bg-gray-100 flex-shrink-0"><ChevronRight size={18}/></button>
                         </>
                    )}
                    <button onClick={handleToday} className="no-print text-xs sm:text-sm font-semibold text-indigo-600 hover:bg-indigo-50 px-2 sm:px-3 py-1.5 rounded flex-shrink-0">Today</button>
                    {(viewMode === 'week' || viewMode === 'calendar') && (
                        <button 
                            onClick={handlePrint} 
                            className="no-print p-2 rounded-full hover:bg-gray-100 flex-shrink-0 text-gray-600 hover:text-gray-800" 
                            title="Print View"
                        >
                            <Printer size={18} />
                        </button>
