...
export const fetchBankHolidays = async (): Promise<Map<string, string[]>> => {
    ...
    try {
        ...
        console.warn("Error fetching bank holidays, using fallback constants.", error);
        
        // Fallback to constants if API fails
        const map = new Map<string, string[]>();
        map.set('england-and-wales', BANK_HOLIDAYS);
        map.set('scotland', BANK_HOLIDAYS); // Fallback to same list for now
        map.set('northern-ireland', BANK_HOLIDAYS);
...
/**
 * Helper to get holiday list for a specific region. Defaults to England & Wales.
 */
export const getHolidaysForRegion = (region: string = 'england-and-wales', allHolidays?: Map<string, string[]>): string[] => {
